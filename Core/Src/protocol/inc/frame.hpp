...
alignas(4) inline uint8_t g_dma_rx_buffer[DMA_RX_BUFFER_SIZE];
inline volatile size_t g_read_pos = 0;
...
struct FrameHeader {
  ...
  static constexpr size_t size() {
    return sizeof(version) + sizeof(type) + sizeof(seq_num) +
           sizeof(payload_len);
  }
};
#pragma pack(pop)
...
struct Frame : public FrameHeader {
  ...
  bool is_version_valid() const { return version == CURRENT_VERSION; }
};
