...
bool FrameParser::validate_crc() {
    ...
    #endif
    calculated_crc = Crc16::calculate(header_bytes, sizeof(header_bytes), calculated_crc);
    if (!rx_frame_.payload.empty()) {
        calculated_crc = Crc16::calculate(rx_frame_.payload.data(), rx_frame_.payload.size(), calculated_crc);
    }
