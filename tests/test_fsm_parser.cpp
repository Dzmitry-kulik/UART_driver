...
std::vector<uint8_t> make_frame(uint8_t version, MessageType type, uint8_t seq,
                                const std::vector<uint8_t> &payload,
                                bool corrupt_crc = false) {
  std::vector<uint8_t> frame = {0xAA, 0x55, version, static_cast<uint8_t>(type),
                                seq};
...
int main() {
  ...
  auto f1 = make_frame(0x01, MessageType::COMMAND, 1, {});
  parser.process_buffer(f1.data(), f1.size());
  assert(g_frames_ok == 1);

  // 3. Успешный кадр с данными
  auto f2 = make_frame(0x01, MessageType::DATA, 2, {0x01, 0x02, 0x03, 0x04});
  parser.process_buffer(f2.data(), f2.size());
  assert(g_frames_ok == 2);

  // 4. Ошибка CRC
  auto f3 = make_frame(0x01, MessageType::DATA, 3, {0xAA}, true);
  ...
  auto f5 = make_frame(0x01, MessageType::DATA, 5, {0x11, 0x22});
  ...
  auto f7 = make_frame(0x01, MessageType::COMMAND, 7, {0x77});
